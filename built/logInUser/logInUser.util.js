...
exports.getRandomYtIdArr = exports.getThisMonthCalendarData = exports.selectFieldForLogIn = exports.getTotalUnreadNotification = exports.getTodayDiaries = void 0;
...
const getTodayDiaries = async (userId) => {
    ...
    //   return true;
    // }
    const todayDiaries = await client_1.default.diary.findMany({
        where: {
...
exports.getTodayDiaries = getTodayDiaries;
const getTotalUnreadNotification = (userId, lastReadNotificationId) => client_1.default.notification.count({
    where: {
...
exports.getTotalUnreadNotification = getTotalUnreadNotification;
exports.selectFieldForLogIn = {
    id: true,
    userName: true,
    avatar: true,
...
const getThisMonthCalendarData = async (userId) => {
    // 이번달 일기도 보냄
    const { nowYear, nowMonth } = (0, yearCheckList_1.getNow)();
    const tableName = "y" + String(nowYear).substring(2, 4);
    const convertedMonth = "m" + nowMonth;
    // const result = await client.y22.findUnique({
    const result = await client_1.default[tableName].findUnique({
        where: {
            userId,
        },
        select: {
...
exports.getThisMonthCalendarData = getThisMonthCalendarData;
...
//   return randomArr;
// };
const getRandomYtIdArr = (ytIdArr) => {
    let randomArr;
    if (ytIdArr) {
        const MAX_YT_NUMBER = 10;
        let ytIdArrLength = ytIdArr.length;
        if (ytIdArrLength > MAX_YT_NUMBER) {
            const getRandomIndex = () => Math.floor(Math.random() * ytIdArrLength);
            const removeYtIdOnArrByIndex = (randomIndex) => {
                ytIdArr.splice(randomIndex, 1);
                ytIdArrLength -= 1;
            };
            if (ytIdArrLength < MAX_YT_NUMBER * 2) {
...
exports.getRandomYtIdArr = getRandomYtIdArr;
