...
const synchronizeDiaryTypeCheck_1 = require("./synchronizeDiaryTypeCheck");
// const synchronizeDiaryFn: Resolver = async(_,{uploadDiaries}:variableType,{client,loggedInUser}) => {
const synchronizeDiaryFn = async (_, { uploadDiaries }, { client, logInUserId }) => {
    // dateTime 만 타입체크 하면 될듯.
    const eachDiaryUploadFnArr = [];
    const calendarUploadObj = {};
    ...
    try {
        uploadDiaries.map(eachDiary => {
            ...
            const { date, year, month, yearSimple, } = (0, synchronizeDiaryTypeCheck_1.dateTimeCheckIfOkReturnFormDateElseError)({
                dateTime,
                nowFullDate,
                nowYear,
                nowMonth,
    ...
    for (const yearTableName in calendarUploadObj) {
        ...
        const selectOnlyChangedMonth = {}; // { m2:true, m4:true, m11:true }
...
const resolver = {
    Mutation: {
        synchronizeDiary: (0, user_utils_1.protectResolver)(synchronizeDiaryFn),
    },
};
exports.default = resolver;
const makeUpdateData = ({ yearTableName, prevYearlyDateArrOnMonthObj, changeMonthArr, changedYearlyDateArrOnMonthObj }) => {
    const dataObj = {};
