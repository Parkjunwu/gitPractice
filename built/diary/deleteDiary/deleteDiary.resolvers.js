...
const deleteDiaryFn = async (_, { id }, { logInUserId, client }) => {
    ...
    try {
        ...
        if (fileJson && typeof fileJson === 'object' && Array.isArray(fileJson)) {
            await Promise.all(fileJson.map((async (fileUrl) => {
                // await async_deletePhotoS3(fileUrl,S3_FOLDER_NAME);
                await (0, AWS_1.async_deletePhotoS3)(fileUrl);
            })));
        }
    }
    catch (e) {
...
const resolver = {
    Mutation: {
        deleteDiary: (0, user_utils_1.protectResolver)(deleteDiaryFn),
    },
};
