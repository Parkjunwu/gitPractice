...
const deleteAccountFn: Resolver = async(_,__,{client,logInUserId}) => {
  ...
  if(diaryFiles){
    diaryFiles.forEach(fileAndThumbNail=>{
      ...
      if(Array.isArray(file)){
        file.forEach((fileUrl:string)=>async_deletePhotoS3(fileUrl)) // async 안해도 될듯.
      }
  ...
  if(avatar) {
    ...
    async_deletePhotoS3(avatar); // async 안해도 될듯.
  }
